...
public class Disco implements Dispositivo{
	...
	/*** Erro de dispositivo: Nao foi possivel apagar conteudo */
	private static final String ERR_CLEAR 					= "Nao foi possivel apagar o conteudo. Arquivo: %s.";
	...
	public Bits8 position() throws MVNException{
		...
		try {
			return new Bits8((int) inFile.getChannel().position());
		} catch (IOException e) {
			throw new MVNException(ERR_IOERROR, this);
		}
	}
	...
	public boolean isErasable() throws MVNException {
		if (podeEscrever()) {
			return true;
		}
		
		throw new MVNException(Dispositivo.ERR_READONLYDEVICE, arquivo.getName());
	}
	...
	public void clear() throws MVNException {
		try {
			...
			throw new MVNException(ERR_CLEAR, arquivo.getName());
		}
	}
} // Disco